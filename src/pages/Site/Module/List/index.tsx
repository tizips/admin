  ...
  const onDelete = (record: APISiteModules.Data) => {
    if (data) {
      ...
      Loop.ById(
        temp,
        record.id,
        (item: APISiteModules.Data) => (item.loading_deleted = true),
      );
      setData(temp);
    }
      ...
      .finally(() => {
        if (data) {
          ...
          Loop.ById(
            temp,
            record.id,
            (item: APISiteModules.Data) => (item.loading_deleted = false),
          );
          setData(temp);
        }
      });
  ...
  const onEnable = (record: APISiteModules.Data) => {
    if (data) {
      ...
      Loop.ById(
        temp,
        record.id,
        (item: APISiteModules.Data) => (item.loading_enable = true),
      );
      setData(temp);
    }

    const enable: APIRequest.Enable = {id: record.id, enable: record.is_enable === 1 ? 2 : 1};
      ...
      .then((response: APIResponse.Response<any>) => {
        if (response.code !== Constants.Success) {
          ...
          notification.success({
            message: `模块${enable.enable === 1 ? '启用' : '禁用'}成功！`,
          });
          if (data) {
            const temp = [...data];
            Loop.ById(temp, record.id, (item: APISiteModules.Data) => (item.is_enable = enable.enable),);
            setData(temp);
          }
        }
      })
