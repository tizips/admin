...
const Tree: React.FC = () => {
  ...
  const doLoop = (items: APISiteCategories.Data[], callback: (item: APISiteCategories.Data) => void) => {
  ...
  return (
    <>
      <Card title='栏目列表' extra={<Row gutter={10}>
        <Col>
          <Tooltip title='刷新'>
        ...
        {
          initialState?.permissions && initialState?.permissions?.indexOf('site.category.create') >= 0 ?
            <Col>
              <Tooltip title='创建'>
        ...
        <Table dataSource={data} rowKey='id' size='small' indentSize={50}
          ...
          <Table.Column title='启用' align='center' render={(record: APISiteCategories.Data) => (
            <Switch size='small' checked={record.is_enable === 1} onClick={() => onEnable(record)}
                    disabled={initialState?.permissions && initialState?.permissions?.indexOf('site.category.enable') < 0}
                    loading={record.loading_enable} />
          )} />
          {
            initialState?.permissions && initialState?.permissions?.indexOf('site.category.update') >= 0 ||
            initialState?.permissions && initialState?.permissions?.indexOf('site.category.delete') >= 0 ?
              <Table.Column title='操作' align='right' width={100} render={(record: APISiteCategories.Data) => (
                <>
                  {
                    initialState?.permissions && initialState?.permissions?.indexOf('site.category.delete') >= 0
                    && !record.children ?
