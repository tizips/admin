...
import Editor from '@/pages/Auth/Admin/Editor';
    ...
    doEnable(enable)
      .then((response: APIResponse.Response<any>) => {
        if (response.code !== Constants.Success) {
          ...
          notification.success({ message: `账号${enable.enable === 1 ? '启用' : '禁用'}成功！` });
          if (data) {
            const temp = [...data];
            Loop.byId(temp, record.id, (item: APIAuthAdmins.Data) => item.is_enable = enable.enable);
            setData(temp);
          }
        }
      })
  ...
  return (
    <>
      <Card title='账号列表' extra={<Row gutter={10}>
        ...
        {
          initialState?.permissions && initialState?.permissions?.indexOf('auth.admin.create') >= 0 ?
            <Col>
              <Tooltip title='创建'>
        ...
        <Table dataSource={data} rowKey='id'
          ...
          <Table.Column title='启用' align='center' render={(record: APIAuthAdmins.Data) => (
            <Switch size='small' checked={record.is_enable === 1} onClick={() => onEnable(record)}
                    disabled={initialState?.permissions && initialState?.permissions?.indexOf('auth.admin.enable') < 0}
          ...
          {
            initialState?.permissions && initialState?.permissions?.indexOf('auth.admin.update') >= 0 ||
            initialState?.permissions && initialState?.permissions?.indexOf('auth.admin.delete') >= 0 ?
